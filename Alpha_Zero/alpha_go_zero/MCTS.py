...
class Node:
    def __init__(self, game, mother=None, prob=torch.tensor(0., dtype=torch.float)):
        ...
        if self.game.score is not None:
            ...
            self.U = 0 if self.game.score is 0 else self.V*float('inf') #this speeds up the training
    ...
    def explore(self, policy):
        #Utilizes the 4 steps of MCTS, but guided by the network policy and value function for much better search efficiency.
        #B'cse it doesn't do MC rollout/simulation, it is more like TDTS and not MCTS.
        #See the comments in the 'simulation' section below for details.

        if self.game.score is not None:
            raise ValueError("game has ended with score {0:d}".format(self.game.score))

        current = self 
        #any modifications made to current while it is pointing to self will also modify self i.e. rootnode
        #This is actually how the search tree is build over each iteration of explore
        ...
        while current.child and current.outcome is None:
            ...
            #this helps speed up the training. Whenever we see a winning move, we don't have to explore the other moves.
            if max_U == -float("inf"): #for next player
                #current outcome=-current.game.player
                current.U = float("inf") #switch to current player with -ve
                current.V = 1.0
                break
            
            elif max_U == float("inf"): #for next player
                #current outcome=current.game.player
                current.U = -float("inf")  #switch to current player with -ve
                current.V = -1.0
                break
        ...
        if not current.child and current.outcome is None:
            ...
            # thus extra - sign is needed for the current player's perspective.
            next_actions, probs, v = process_policy(policy, current.game)
            current.nn_v = -v
            current.create_child(next_actions, probs) #will create all the children at once and not just one child
            current.V = -float(v)

        
        current.N += 1

        #3. Simulation /roll out step: not doing as its very expensive to evaluate the policy network for this
        # you can think of it as just a single step roll out since expansion is like the first step in roll out. It's sort of like TD estimate rather than MC estimate of the game score
        # without full MC rollout, this is really like a single step TD rollout. It is really TDTS instead of MCTS.
        ...
        while current.mother:
            ...
            #mother.V += (-current.V - mother.V)/mother.N #original version but I think it is wrong (this is like TD update) (running average)
            mother.V += (-current.V - (mother.N-1)*mother.V)/mother.N #my modified version (this is like TD update) (running average)
            # Note: nn_v is not backpropagated. It is only updated when the state is being expanded (step 2) using the policy network.
        ...
        #return sel_depth, debug_find_max_tree_depth(current) #for debug only (depth during selection vs tree depth at the end of explore)
    ...
    def next(self, temperature=1.0):
        ...
        if max_U == float("inf"): # if there are winning moves, just output those
        ...
        # V was for the current player making a move
        # to convert to the next player we add - sign
...
def debug_find_max_tree_depth(current):
    children = current.child
    if children is not {}:
        max_len = -1
        for a_,c_ in children.items():
            c_len = 1+debug_find_max_tree_depth(c_)
            if max_len < c_len: max_len = c_len
        return max_len            
    else:
        max_len = 0
        return max_len
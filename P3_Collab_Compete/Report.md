...
The network was trained using Adam optimizer with elu non-linearity for faster training. I performed a few things to help speed up the learning process: 1) for the first 300 episodes, no learning occurred. The agents were just doing random exploration, 2) thereafter, for each step in the environment, each agent performed three learning iterations, and 3) the additive noise introduced by a normal distrubution for better exploration was gradually decayed down to 30% of the noise amount used at beginning of training. Additionally, using a normal distribution for exploration was found to be much more effective than using the Ornstein–Uhlenbeck noise process.

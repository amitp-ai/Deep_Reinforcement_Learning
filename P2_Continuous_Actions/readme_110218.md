...
And so, if the trajector &tau; is sampled from P<sub>&theta;</sub>(&tau;), then the Monte-Carlo estimate of Q<sub>P<sub>&theta;</sub></sub>(s<sub>t</sub>, a<sub>t</sub>) is just:

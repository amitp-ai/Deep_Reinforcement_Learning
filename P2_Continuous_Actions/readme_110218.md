...
In addition to using the rewards to go (due to causality), another approach to minimizing the values of &gcirc; is by subtracting out a fixed number that is not dependend on &theta; or action a. It can be mathematically proved that such a transofrmation is unbiased and does indeed reduce variance. And intuitive explanation for why it reduces variance is because the terms XXX has smaller magnitude, which will reduce the variance of the overall expression.

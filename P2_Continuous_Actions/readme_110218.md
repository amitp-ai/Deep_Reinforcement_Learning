...
Note that the reason why the above estimate is valid is because the trajectories are generated by following the policy being learned, i.e. P<sub>&theta;</sub>(&tau;). Another way to say it is that we sample the trajectories &tau;<sup>i</sup> from the probability distribution P<sub>&theta;</sub>(&tau;).

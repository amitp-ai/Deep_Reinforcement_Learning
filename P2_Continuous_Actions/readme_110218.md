...
One problem with the above Equation is that it is very difficult to compute the above expectation. Hence, the state-value function is modelled with a separate neural network that is parameterized by w<sub>v</sub> as follows:

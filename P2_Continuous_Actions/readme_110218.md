...
In addition to using the rewards to go (due to causality), another approach to minimizing the variance of &gcirc; is by subtracting out a fixed number that is not dependend on &theta; or action a. It can be mathematically proved that such a transformation is unbiased and reduces variance. An intuitive explanation for why it reduces variance is because the terms multiplying &nabla;<sub>&theta;</sub>log(P<sub>&theta;</sub>(a|s)) has smaller magnitude, which essentially reduces the variance of the overall expression.

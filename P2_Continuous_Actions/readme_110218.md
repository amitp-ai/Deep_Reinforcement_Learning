...
<img src="https://latex.codecogs.com/png.latex?\fn_cm&space;\nabla_{\theta}U_{\theta}&space;=&space;E_{\tau&space;\,&space;\sim&space;\,&space;P_{\theta}(\tau)}&space;\left[&space;\left(&space;\sum_{t=0}^{T}r(s_{t},a_{t})&space;\right)\left(\sum_{t=0}^{T}\nabla_{\theta}log(P_{\theta}(a_{t}|s_{t}))&space;\right)&space;\right]&space;\:&space;...&space;\,&space;Equation&space;\,&space;3" title="\nabla_{\theta}U_{\theta} = E_{\tau \, \sim \, P_{\theta}(\tau)} \left[ \left( \sum_{t=0}^{T}r(s_{t},a_{t}) \right)\left(\sum_{t=0}^{T}\nabla_{\theta}log(P_{\theta}(a_{t}|s_{t})) \right) \right] \: ... \, Equation \, 3" />
...
1. Start with a random vector &theta;
2. Until Convergence:
      a. Use the policy P<sub>&theta;</sub>(a<sub>t</sub>|s<sub>t</sub>) to collect m trajectories {&tau;<sup>1</sup>, &tau;<sup>2</sup>, ..., &tau;<sup>m</sup>}, where each trajectory is as defined above.
      b. Use these trajectories to compute the Monte-Carlo estimate of the gradient as follows:

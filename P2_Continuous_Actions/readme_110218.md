...
Goal in RL is to maximize the expected total utility U<sub>&theta;</sub>, which is defined as follows:
...
<img src="https://latex.codecogs.com/png.latex?\fn_cm&space;TD&space;\;&space;Target&space;=&space;r(s_t,a_t)&space;&plus;&space;\gamma&space;\max_a\hat{Q}_{P_\theta}(s_{t&plus;1},a,w)&space;-&space;\hat{Q}_{P_\theta}(s_{t},a_{t},w)&space;\:&space;...&space;\,&space;Equation&space;\,&space;17" title="TD \; Target = r(s_t,a_t) + \gamma \max_a\hat{Q}_{P_\theta}(s_{t+1},a,w) - \hat{Q}_{P_\theta}(s_{t},a_{t},w) \: ... \, Equation \, 17" />
...
In addition to using the rewards to go (due to causality), another approach to minimizing the variance of &gcirc; is by subtracting out a baseline b that is not dependend on &theta; or action a. It can be mathematically proved that such a transformation is unbiased and reduces variance. An intuitive explanation for why it reduces variance is because the term multiplying &nabla;<sub>&theta;</sub>log(P<sub>&theta;</sub>(a|s)) has smaller magnitude, which essentially reduces the variance of the overall expression.
...
Whereby
There are many choices for the baseline b, and in theory, the optimal value of b can also be computed. However, in the interest of simplicity and intuition, a commonly used baseline is the q-value averaged over all the actions, i.e. the state-value.
...
The Advantage function is defines as follows:

<img src="https://latex.codecogs.com/gif.latex?\hat{A}_{P_\theta}(s_t,&space;a_t)&space;=&space;\hat{Q}_{P_\theta}(s_t,&space;a_t,&space;w)&space;-&space;\hat{V}_{P_\theta}(s_t,&space;w)" title="\hat{A}_{P_\theta}(s_t, a_t) = \hat{Q}_{P_\theta}(s_t, a_t, w) - \hat{V}_{P_\theta}(s_t, w)" />
...
<img src="https://latex.codecogs.com/png.latex?\fn_cm&space;\hat{V}(s_t,&space;w_v)&space;\approx&space;\hat{V}_{P_\theta}(s_t,&space;w)&space;\:&space;...&space;\,&space;Equation&space;\,&space;20" title="\hat{V}(s_t, w_v) \approx \hat{V}_{P_\theta}(s_t, w) \: ... \, Equation \, 20" />

The advantage function &Acirc;<sub>P<sub>&theta;</sub></sub>(s<sub>t</sub>, a<sub>t</sub>) now becomes:

<img src="https://latex.codecogs.com/gif.latex?\hat{A}_{P_\theta}(s_t,&space;a_t)&space;=&space;\hat{Q}_{P_\theta}(s_t,&space;a_t,&space;w)&space;-&space;\hat{V}_{P_\theta}(s_t,&space;w)&space;\:&space;...&space;\,&space;Equation&space;\,&space;21" title="\hat{A}_{P_\theta}(s_t, a_t) = \hat{Q}_{P_\theta}(s_t, a_t, w) - \hat{V}_{P_\theta}(s_t, w) \: ... \, Equation \, 21" />

<img src="https://latex.codecogs.com/gif.latex?\hat{A}_{P_\theta}(s_t,&space;a_t)&space;\approx&space;\hat{A}(s_t,&space;a_t,&space;w,&space;w_v)&space;=&space;\hat{Q}_{P_\theta}(s_t,&space;a_t,&space;w)&space;-&space;\hat{V}(s_t,&space;w_v)&space;\:&space;...&space;\,&space;Equation&space;\,&space;22" title="\hat{A}_{P_\theta}(s_t, a_t) \approx \hat{A}(s_t, a_t, w, w_v) = \hat{Q}_{P_\theta}(s_t, a_t, w) - \hat{V}(s_t, w_v) \: ... \, Equation \, 22" />



For stochastic policies in continuous environments, the actor outputs the mean and variance of a Gaussian distribution. And an action is sampled from this Gaussian distribution.

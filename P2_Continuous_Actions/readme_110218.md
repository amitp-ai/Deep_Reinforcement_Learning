...
<img src="https://latex.codecogs.com/png.latex?\fn_cm&space;U_{\theta}&space;=&space;E_{s_{0},&space;a_{0}&space;\,&space;\sim&space;\,&space;P(s_{0},&space;a_{0})}&space;\left&space;[&space;Q_\theta(s_{0},&space;a_{0})&space;\right&space;]&space;\:&space;...&space;\,&space;Equation&space;\,&space;1b" title="U_{\theta} = E_{s_{0}, a_{0} \, \sim \, P(s_{0}, a_{0})} \left [ Q_\theta(s_{0}, a_{0}) \right ] \: ... \, Equation \, 1b" />
...
1. Using Causality: &gcirc; updates all the actions in a trajectory based upon total rewards and not the rewards to go. That is to say, past rewards are affected by future actions, which is not possible in our causal Universe. So we can make the gradeint estimate more realistic by using rewards to go as shown in the below equation.

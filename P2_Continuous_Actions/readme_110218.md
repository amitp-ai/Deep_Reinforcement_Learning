...
For any visited state-action pair (s,a), the actor network is updated using Equation 6 (utilizing &gcirc; from Equation 12), and the critic network is typically updated using TD-learning (due to its lower variance than using Monte-Carlo learning) using the following update equation:
